...
# -- Step1: Create and print Instances
...
# ----- 2. Addition
    def __add__(self, other):
        top = (self.deno * other.num) + (self.num * other.deno)
        bottom = self.deno * other.deno
        return Fraction(top,bottom)
# ----- 3. Division
    def __truediv__(self, other):
        # -- a/b divided by c/d means a/b * d/c
        top = self.num * other.deno
        bottom = self.deno * other.num
        return Fraction(top,bottom)
# ----- 4. cast Fraction object to float to get the value of like 4/2 = 2.0 
    def __float__(self):
        return self.num/self.deno
    
# -- Step 3: reducing the Fraction like 2/12 or kindoff 
    def reduce(self):
        def gcd(n,d):
            while d != 0:
                (d,n) = (n%d,d)
        ...
        if self.deno == 0:
            ...
            # return self.num # -- if this loop becomes TRUE the output will of datatype 'int' instead of 'fraction'
            # -- we cant do 4 *(1/2) since 4 is int and 1/2 is a fraction so for that we can do
            return Fraction(self.num,1)
        else:
            greatest_common_factor = gcd(self.num,self.deno)
            top = int(self.num/greatest_common_factor)
            bottom = int(self.deno/greatest_common_factor)
            return Fraction(top,bottom)

# -- Check the step1 is working or not 
f1 = Fraction(4,1)
f2 = Fraction(1,4)
f3 = Fraction(5,1)

print(f"Reduce function usage: {f1} = {f1.reduce()}")

print(f"Representation of fraction f1,f2,f3 is {f1},{f2},{f3}")

# -- Checking step2 (__mul__)
print(f"Multiplication of {f1} and {f2} is {f1*f2}")
print(f"Multiplication of {f1} and {f2} is {float(f1*f2)}")

# -- Checking Step2 (__Add__)
print(f"Addition of {f1} and {f2} is {f1+f2}")
print(f"Addition of {f1} and {f2} is {float(f1+f2)}")

# -- Checking Step2(__truediv__)
print(f"Division of {f1} and {f2} is {f1/f2}")
print(f"Division of {f1} and {f2} is {float(f1/f2)}")
